...
interface Props {
  ...
  /** Denser layout for use in a multi-route grid instead of a single hero card. */
  compact?: boolean
  /** Offsets the wave's phase so tickers shown side by side don't move in lockstep. */
  phaseSeed?: number
}
...
function occupancyWave(t: number, seed: number): number {
  return 6 * Math.sin(t / 5.3 + seed) + 3.5 * Math.sin(t / 2.1 + 1.4 + seed * 1.7) + 1.8 * Math.sin(t / 0.9 + 0.6 + seed * 2.3)
}
function competitorWave(t: number, seed: number): number {
  return 0.06 * Math.sin(t / 7.7 + 2.2 + seed) + 0.03 * Math.sin(t / 3.1 + seed * 1.3)
}
...
export default function LiveTicker({
  ...
  useEffect(() => {
    function loop(now: number) {
      if (now - lastTickRef.current >= TICK_MS) {
        ...
        const occupancyPct = Math.max(2, Math.min(99, baseOccupancy + occupancyWave(t, phaseSeed)))
        const competitorAvg = baseCompetitor * (1 + competitorWave(t, phaseSeed))
  ...
  return (
    <div className={`rounded-xl border border-slate-800 bg-slate-900 text-white relative overflow-hidden ${compact ? 'p-3.5' : 'p-5'}`}>
      <div
        className="pointer-events-none absolute -top-24 -right-24 w-72 h-72 rounded-full opacity-20 blur-3xl"
        style={{ background: 'radial-gradient(circle, #6366f1, transparent 70%)' }}
      />
      <div className={`relative flex items-center justify-between flex-wrap gap-y-1 ${compact ? 'mb-2' : 'mb-3'}`}>
        <div className="flex items-center gap-1.5 min-w-0">
          <span className="relative w-1.5 h-1.5 rounded-full bg-red-500 shrink-0 ticker-dot" />
          <span className="text-[9px] font-bold tracking-[0.15em] text-red-400 uppercase shrink-0">Live</span>
          <span className={`text-slate-200 font-medium truncate ${compact ? 'text-xs' : 'text-sm'}`}>{label}</span>
        </div>
        {!compact && <span className="text-[11px] text-slate-500 truncate">{sublabel}</span>}
      </div>
      {compact && <div className="relative text-[10px] text-slate-500 truncate -mt-1.5 mb-2">{sublabel}</div>}

      <div className="relative flex items-end justify-between mb-1 flex-wrap gap-3">
        <div>
          <div
            key={current.price}
            className={`font-bold font-display tabular-nums leading-none ${compact ? 'text-xl' : 'text-3xl sm:text-4xl'}`}
            style={{ color: dirColor, animation: 'tickerFlash .45s ease' }}
          >
            {currency(current.price)}
          </div>
          <div className={`mt-1.5 font-medium ${compact ? 'text-[10px]' : 'text-xs'}`} style={{ color: dirColor }}>
            {dirArrow} {deltaPct >= 0 ? '+' : ''}
            {deltaPct.toFixed(1)}%{compact ? '' : ` vs base fare ${currency(basePrice)}`}
          </div>
        </div>
        <div className="text-right">
          <div className="text-[9px] text-slate-500 uppercase tracking-wide">Demand</div>
          <div className={`font-semibold text-slate-100 ${compact ? 'text-base' : 'text-lg'}`}>
            {current.demand}
      ...
      <div className={`relative ${compact ? 'mt-1' : 'mt-2'}`}>
        ...
        >
          <defs>
            <linearGradient id={`tickerFill-${label}`} x1="0" y1="0" x2="0" y2="1">
          ...
          />
          {pts.length > 1 && <polygon points={areaPoints} fill={`url(#tickerFill-${label})`} stroke="none" />}
          {pts.length > 1 && (
            <polyline
              points={linePoints}
              fill="none"
          ...
          {last && <circle cx={xFor(last.time)} cy={yFor(last.price)} r={compact ? 1.6 : 1.3} fill={dirColor} />}
        </svg>
        {!compact && (
